...
class MovieSpider(CrawlSpider):
    ...
    def parse_query_page(self, response):
        ...
        for item in items:
            ...
            data['id'] = self.index # item.xpath('.//h3[@class="lister-item-header"]/span[@class="lister-item-index unbold text-primary"]/text()').get(default='').split('.')[0]
            ...
            data['description'] = item.xpath('.//p[@class="text-muted"]/text()').get(default='').strip() + ". " + lorem.paragraph()
            director = item.xpath('.//p[@class]/a/text()').get(default='').strip()
            if director != 'See full summary' and director != "Add a Plot":
